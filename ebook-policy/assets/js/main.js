...
(function (window, $, Backbone) {
  ...
  var FlipbookView = SingleView.extend({
    ...
    _updateInlineShadows: function () {
      var self = this;
      var isSingle = self.$el.turn('display') === 'single';
      var totalPages = self.$el.turn('pages');

      self.$el.find('.page-wrapper').each(function () {
        var $pageWrapper = $(this);
        var pageNum = parseInt($pageWrapper.attr('page'), 10);
        var $shadow = $pageWrapper.find('.page-inline-shadow');

        if (isSingle || pageNum === 1 || pageNum === totalPages) {
    ...
    _getPageElement: function (pageNumber) {
      ...
      var $el = $('<div />'),
        ...
        imgSrc = settings.pageFolder + '/page-' + pageNumber + '.png';

      // Create a wrapper to hold the image and the shadow
      var $wrapper = $('<div />', {
        ...
        css: {
          ...
          // boxShadow: '5px 5px 20px rgba(0,0,0,0.2)' // Add shadow behind the book
        }
      });
      ...
      var $img = $('<img />', {
        ...
        css: {
          ...
          position: 'relative',
          zIndex: 1,
          // boxShadow: isRightPage ? 'inset -5px 0 10px rgba(0,0,0,0.1)' : 'inset 5px 0 10px rgba(0,0,0,0.1)'
        }
      });

      // Determine shadow properties based on page number
      var isRightPage = pageNumber % 2 === 1;
      var shadowSide = isRightPage ? 'left' : 'right';
      var shadowGradient = isRightPage ?
        'linear-gradient(to right, rgba(0,0,0,0.2) 0%, rgba(0,0,0,0) 20%)' :
        'linear-gradient(to left, rgba(0,0,0,0.2) 0%, rgba(0,0,0,0) 20%)';

      var $shadow = $('<div />', {
        'class': 'page-inline-shadow shadow-' + shadowSide,
        css: {
          position: 'absolute',
          top: 0,
          bottom: 0,
          ...
          left: shadowSide === 'left' ? 0 : 'auto',
          right: shadowSide === 'right' ? 0 : 'auto'
        }
      });

      // Add center black line
      var $centerLine = $('<div />', {
        'class': 'page-center-line',
        css: {
          position: 'absolute',
          top: 0,
          bottom: 0,
          width: '0.1px', // 1px thin black line
          backgroundColor: '#b6b6b6ff',
          pointerEvents: 'none',
          zIndex: 3,
          opacity: 1,
          left: isRightPage ? 0 : 'auto',
          right: !isRightPage ? 0 : 'auto'
        }
      });
      ...
      $img.on('load', function (event) {
        ...
        // Ensure shadow is visible immediately
        $wrapper.find('.page-inline-shadow').css('opacity', 1);
      });


      $img.attr('src', imgSrc);
      // Assemble the page element
      $wrapper.append($img).append($shadow).append($centerLine);
      $el.append($wrapper);
      $el.attr('data-page', pageNumber); // Add data-page for easier selection
  ...
  var MiniaturesView = SingleView.extend({
    ...
    _refreshPictureEvent: function (event, pageNumber, $pageElement) {
      var settings = window.FlipbookSettings,
        imgSrc = settings.pageFolder + '/thumb-' + pageNumber + '.png',
...
FlipbookSettings = {
  ...
  shareMessage: 'ข้อบังคับเกี่ยวกับการทำงาน บริษัท ฮีดากาโยโก เอ็นเตอร์ไพรส์ จำกัด',
